...
def snake_bspline(t_coeff, l_centerline=1.0, keep_pts=False):
    ...
    keep_pts : boolean, optional
        ...
        by returned as a (3, ) tuple. If False, returns only the spline object.
        Defaults to False.
...
def test_bspline(betas, number_of_nodes, total_length):
    """ Test object for the bspline function"""
    l_centre = total_length
    #t_coeff = np.abs(np.random.randn(6, ))
    #t_coeff = np.array([0.1, 0.3, 0.15, 0.22, 0.25, 0.1])

    #t_coeffs from the paper:
    t_coeff = np.array([0, betas[0], betas[1], betas[2], betas[3], 0])
    my_spline, ctr_pts, ctr_coeffs = snake_bspline(t_coeff, total_length, keep_pts=True)
    #my_spline(0.2)

    #from matplotlib import pyplot as plt
    s = np.linspace(0.0, total_length, number_of_nodes+1)
    s_actual = (s[1:] + s[:-1])/2.
    return my_spline(s_actual)
...
if __name__ == "__main__":
    print(test_bspline([10, 20, 30, 40.], 10, 3.))
